...
- [node](https://hub.docker.com/_/node)
    ![](https://img.shields.io/badge/node_slim_-12.14.0-21cb00.svg)
...
```bash
docker build -t esdoc-converter:12 ./12
```

### run container

```bash
docker run --rm -v $PWD:/root/app esdoc-converter:12
```

## Use Dockerhub image

```bash
docker pull blackellis/dokcer-esdoc-converter:12
docker run --rm -v $PWD:/root/app blackellis/dokcer-esdoc-converter:12
```
